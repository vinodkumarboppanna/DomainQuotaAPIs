...
from novaclient.v1_1 import domain_quotas
...
_domain_quota_resources = ['instances', 'cores', 'ram', 
                    'floating_ips', 'fixed_ips', 'metadata_items',
                    'injected_files', 'injected_file_content_bytes',
                    'injected_file_path_bytes', 'key_pairs',
                    'security_groups', 'security_group_rules']

def _domain_quota_show(domain_quotas):
    class FormattedDomainQuota(object):
        def __init__(self, key, value):
            setattr(self, 'quota', key)
            setattr(self, 'limit', value)

    quota_list = []
    for resource in _domain_quota_resources:
        try:
            quota = FormattedDomainQuota(resource, getattr(domain_quotas, resource))
            quota_list.append(quota)
        except AttributeError:
            pass
    columns = ['Quota', 'Limit']
...
@utils.arg('domain',
    metavar='<domain-id>',
    help='ID of domain to list the quotas for.')
@utils.arg('--tenant',
    metavar='<tenant-id>',
    default=None,
    help='ID of tenant to list the quotas for.')
@utils.arg('--user',
    ...
    default=None,
    help='ID of user to list the quotas for.')
def do_domain_quota_show(cs, args):
    ...
    if args.user:
       user_ud = args.user
    _domain_quota_show(cs.domain_quotas.get(domain_id=args.domain, tenant_id=tenant_id, user_id=user_id))

@utils.arg('domain',
    metavar='<domain-id>',
    help='ID of domain to list the default quotas for.')
def do_domain_quota_defaults(cs, args):
    """List the default quotas for a domain."""
    _domain_quota_show(cs.domain_quotas.defaults(args.domain))

def _domain_quota_update(manager, identifier, args):
    updates = {}
    for resource in _domain_quota_resources:
        val = getattr(args, resource, None)
        if val is not None:
    ...
    if updates:
        ...
        user_id = getattr(args, 'user', None)
        if isinstance(manager, domain_quotas.DomainQuotaSetManager):
            manager.update(identifier, tenant_id=tenant_id, user_id=user_id, **updates)
        else:
            manager.update(identifier, **updates)

@utils.arg('domain',
           metavar='<domain-id>',
           help='ID of domain to set the quotas for.')
@utils.arg('--tenant',
           metavar='<tenant-id>',
           default=None,
           help='ID of tenant to set the quotas for.')
...
def do_domain_quota_update(cs, args):
    """Update the quotas for a domain/tenant/user."""

    _domain_quota_update(cs.domain_quotas, args.domain, args)

@utils.arg('domain',
           metavar='<domain-id>',
           help='ID of domain to delete quota for.')
@utils.arg('--tenant',
           metavar='<tenant-id>',
           default=None,
           help='ID of tenant to delete quota for.')
@utils.arg('--user',
           ...
           default=None,
           help='ID of user to delete quota for.')
def do_domain_quota_delete(cs, args):
    """Delete quota for a domain/tenant/user so their quota will Revert
       back to default.
    """
    cs.domain_quotas.delete(args.domain, tenant_id=args.tenant, user_id=args.user)
