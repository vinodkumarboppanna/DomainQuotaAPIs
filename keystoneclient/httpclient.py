...
#vinod here Written by termie for OCL
...
def request(url, method='GET', headers=None, original_ip=None, debug=False,
    ...
    _logger.debug("final url is %s", url)
    if not headers:
        headers = dict()
...
class HTTPClient(object):
    ...
    def __init__(self, username=None, tenant_id=None, tenant_name=None,
        ...
        if auth_ref:
            _logger.debug("enterring into auth_ref")
            ...
            _logger.debug("self management url is %s", self.management_url)
        ...
        _logger.debug("entering into the httpclient")
        _logger.debug("auth_url %s", auth_url)
        if tenant_id:
    ...
    def process_token(self):
        ...
        if self.auth_ref.project_scoped:
            ...
            if self.management_url is None and self.auth_ref.management_url:
                self.management_url = self.auth_ref.management_url[0]
                ### vinod here
                if isinstance(self.management_url, six.string_types):
		   self.management_url = strip_version_from_url(self.management_url)
                ### upto here
    ...
    def _cs_request(self, url, method, **kwargs):
        ...
        _logger.debug("kwargs are %s", kwargs)
        is_management = kwargs.pop('management', True)
        _logger.debug("url in cs request is %s", url)
        _logger.debug("is management in cs request is %s", is_management)
        ...
        _logger.debug("url_to_use in cs request is %s", url_to_use)
        management_is = None
        ...
        _logger.debug("url_to_use in cs request is %s", url_to_use)
        _logger.debug("self management url in cs request is %s", self.management_url)
        _logger.debug("self version is %s", self.version)
    ...
    def get(self, url, **kwargs):
        _logger.debug("vinci here %s", url)
        _logger.debug("kwargs in httpclient are %s", kwargs)
