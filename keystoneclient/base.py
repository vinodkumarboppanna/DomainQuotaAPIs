...
class Manager(object):
    ...
    def __init__(self, client):
        ...
        self.client = client
        _logger.debug("vivek client is %s", client);
    ...
    def _list(self, url, response_key, obj_class=None, body=None, **kwargs):
        ...
        if body:
            ...
            _logger.debug("self client is %s", self.client)
            resp, body = self.client.get(url, management_is=management_is)

        _logger.debug("vivek url is %s", url);
    ...
    def _get(self, url, response_key):
        ...
        :param response_key: the key to be looked up in response dictionary,
            e.g., 'server'
        """
        _logger.debug("vivek1 url is %s", url);
    ...
    def _post(self, url, body, response_key, return_raw=False):
        ...
        """
        _logger.debug("post url is %s", url)
        _logger.debug("post client is %s", client)
        resp, body = self.client.post(url, body=body)
        if return_raw:
...
class CrudManager(Manager):
    ...
    def list(self, **kwargs):
        _logger.debug("kwargs in base is %s", kwargs)
        url = self.build_url(dict_args_in_out=kwargs)
        return self._list(
            '%(url)s%(query)s' % {
                'url': url,
