...
def quota_get_all_by_project_and_user(context, project_id, user_id):
    ...
    user_quotas = model_query(context, models.ProjectUserQuota.resource,
                       ...
                       base_model=models.ProjectUserQuota, read_deleted="no").\
                   filter_by(project_id=project_id).\
                   filter_by(user_id=user_id).\
                   all()
...
def quota_get_all(context, project_id):
    ...
    result = model_query(context, models.ProjectUserQuota, read_deleted="no").\
                   filter_by(project_id=project_id).\
                   all()
...
@require_context
def quota_get_all_for_user(context, project_id, user_id):
    nova.context.authorize_project_context(context, project_id)
...
@require_context
def quota_get_all_for_domain(context, domain_id, project_id):
    nova.context.authorize_domain_context(context, domain_id)
...
def quota_create(context, project_id, resource, limit, user_id=None):
    ...
    quota_ref.created_at = timeutils.utcnow()
    try:
        quota_ref.save()
    except db_exc.DBDuplicateEntry:
...
@require_admin_context
def domain_quota_create(context, domain_id, resource, limit):
...
def quota_update(context, project_id, resource, limit, user_id=None):
    ...
    result = query.update({'hard_limit': limit, 'updated_at': timeutils.utcnow()})
    if not result:
        if per_user:
            raise exception.ProjectUserQuotaNotFound(project_id=project_id,
                                                     user_id=user_id)
...
@require_admin_context
def domain_quota_update(context, domain_id, resource, limit):
    model = models.DomainQuota
    query = model_query(context, model).\
                filter_by(domain_id=domain_id).\
                filter_by(resource=resource)
...
@require_admin_context
def quota_destroy_all_by_domain(context, domain_id):
    session = get_session()
    with session.begin():
        model_query(context, models.DomainQuota, session=session,
                    read_deleted="no").\
                filter_by(domain_id=domain_id).\
                soft_delete(synchronize_session=False)

        model_query(context, models.DomainQuotaUsage, session=session,
                    read_deleted="no").\
                filter_by(domain_id=domain_id).\
                soft_delete(synchronize_session=False)

        model_query(context, models.DomainReservation,
                    session=session, read_deleted="no").\
                filter_by(domain_id=domain_id).\
                soft_delete(synchronize_session=False)
