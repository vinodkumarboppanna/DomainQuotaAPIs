...
class DomainQuotaDriver(object):
    """
    Driver to perform necessary checks to enforce domain quotas and obtain
    quota information.  The default driver utilizes the local
    database.
    """
    def get_by_project_and_user(self, context, project_id, user_id, resource):
    ...
    def get_user_quotas(self, context, resources, project_id, user_id,
        ...
        user_quotas = db.quota_get_all_by_project_and_user(context,
                                                           project_id, user_id)
        if (not defaults) and (not usages):
            return user_quotas
        user_usages = None
        if usages:
            user_usages = db.quota_usage_get_all_by_project_and_user(context,
    ...
    def _get_quotas(self, context, resources, keys, has_sync,
        ...
        # Grab and return the quotas (without usages)
        domain_quotas = self.get_domain_quotas(context, sub_resources,
    ...
    def get_domain_quotas(self, context, resources, domain_id,
        ...
        if (not defaults) and (not remains) and (not usages):
    ...
    def _process_quotas(self, context, resources, domain_id, quotas,
        ...
        # matches the one in the context, we use the quota_class from
        # the context
        if domain_id == context.domain_id:
            quota_class = context.quota_class
        if quota_class:
            class_quotas = db.quota_class_get_all_by_name(context, quota_class)
        else:
        ...
        LOG.debug(_("class_quotas1 : %s") % class_quotas)
        default_quotas = self.get_defaults(context, resources)

        LOG.debug(_("default_quotas3 : %s") % default_quotas)
        for resource in resources.values():
            # Omit default/quota class values
            if not defaults and resource.name not in quotas:
                continue
        ...
        if remains:
            project_list = []
            #auth_url = "http://128.142.144.36:35357/v3/"
            auth_url = CONF.keystone_auth_url
            LOG.debug(_("keystone auth url : %s") % auth_url)
            keystone = client.Client(token=context.auth_token,
                                 auth_url=auth_url,
                                 project_id = context.project_id)

            project_list = keystone.projects.list(domain=domain_id)
            LOG.debug(_("project List : %s") % project_list)
            project_ids = []
            if project_list:
               for project in project_list:
                  project_ids.append(project.id)
            LOG.debug(_("project ids : %s") % project_ids)

            if project_ids:
               for id in project_ids:
                  all_quotas = db.quota_get_all_for_domain(context, domain_id, id)
                  for quota in all_quotas:
                     if quota.resource in modified_quotas:
                        modified_quotas[quota.resource]['remains'] -= \
                              quota.hard_limit
    ...
    def get_project_quotas(self, context, resources, project_id,
        ...
        """
        project_quotas = db.quota_get_all_by_project(context, project_id)
        if (not defaults) and (not usages) and (not remains):
            return project_quotas
        project_usages = None
        if usages:
            project_usages = db.quota_usage_get_all_by_project(context,
                                                               project_id)
        return self._process_project_quotas(context, resources, project_id,
                                    project_quotas, quota_class,
                                    defaults=defaults, usages=project_usages,
                                    remains=remains)

    def _process_project_quotas(self, context, resources, project_id, quotas,
                        quota_class=None, defaults=True, usages=None,
                        remains=False):
        modified_quotas = {}
        # Get the quotas for the appropriate class.  If the project ID
    ...
    def get_settable_quotas(self, context, resources, domain_id, project_id=None, user_id=None, usages=None, remains=None):
        """
        Given a list of resources, retrieve the range of settable quotas for
        the given domain or project or user.
        ...
        if (user_id == None) and (project_id == None):
            domain_quotas = self.get_domain_quotas(context, resources,
                                                 domain_id, usages=usages, remains=remains)
            for key, value in domain_quotas.items():
                minimum = max(int(value['limit'] - value['remains']),
                              int(value['in_use'] + value['reserved']))
                settable_quotas[key] = dict(minimum=minimum, maximum=-1, limit=value['limit'], remains=value['remains'], in_use=value['in_use'], reserved=value['reserved'])
        else:
            if user_id:
                ### setting the quotas for a user

                ### This is called to check that the quotas for the parent node i.e project has set or not
                project_quotas = self.get_project_quotas(context, resources, project_id, defaults=False, usages=False, remains=False)
                if len(project_quotas) <= 1:
                     ## because the project_quotas dict may have project_id as one key
                     return project_quotas
                ...
                for resource in resources.values():
                     ...
                     if resource.name not in project_quotas:
                        notFound_Keys.append(resource.name)

                ## This is needed to get the maximum values that can be set to quota resources
                ...
                for key, value in user_quotas.items():
                    ...
                    maximum = -1
                    if key in project_quotas1:
                        maximum = int(project_quotas1[key]['remains'])
                        if key in user_quotas1:
                            maximum += int(user_quotas1[key])
                    settable_quotas[key] = dict(minimum=minimum, maximum=maximum, limit=value['limit'], in_use=value['in_use'], reserved=value['reserved'])
                settable_quotas['notfound_keys'] = notFound_Keys  
            else:
                ### setting the quotas for the project

                ### This is called to check that the quotas for the parent node i.e domain has set or not
                domain_quotas = self.get_domain_quotas(context, resources,
                                                 domain_id, usages=False, remains=False, defaults=False)
                if len(domain_quotas) <= 1:
                     ## because the domain_quotas dict may have domain_id as one key
                     return domain_quotas
             
                notFound_Keys=[]
                for resource in resources.values():
                     # Omit default/quota class values
                     if resource.name not in domain_quotas:
                        notFound_Keys.append(resource.name)

                ### This is needed to get the maximum values that can be set to quota resources
                domain_quotas1 = self.get_domain_quotas(context, resources, domain_id, usages=True, remains=True, defaults=True)
                LOG.debug(_("domain quotas1 : %s") % domain_quotas1)

                project_quotas = self.get_project_quotas(context, resources, project_id, remains=True)
                project_quotas1 = self.get_project_quotas(context, resources, project_id, defaults=False, usages=False, remains=False)

                for key, value in project_quotas.items():
                    minimum = max(int(value['limit'] - value['remains']),
                              int(value['in_use'] + value['reserved']))
                    maximum = -1
                    if key in domain_quotas1:
                        maximum = int(domain_quotas1[key]['remains'])
                        if key in project_quotas1:
                            maximum += int(project_quotas1[key])
                    settable_quotas[key] = dict(minimum=minimum, maximum=maximum, limit=value['limit'], remains=value['remains'], in_use=value['in_use'], 
    ...
    def destroy_all_by_domain(self, context, domain_id):
        """
        Destroy all quotas, usages, and reservations associated with a
        domain.

        :param context: The request context, for access checks.
        :param domain_id: The ID of the domain being deleted.
        """
        project_list = []
        #auth_url = "http://128.142.144.36:35357/v3/"
        auth_url = CONF.keystone_auth_url
        ...
        db.quota_destroy_all_by_domain(context, domain_id)
...
class QuotaEngine(object):
    ...
    def get_domain_quotas(self, context, domain_id, quota_class=None,
        ...
        LOG.debug(_("resources are : %s") % self._resources)
        return self._driver_domain.get_domain_quotas(context, self._resources,
                                              domain_id,
                                              quota_class=quota_class,
                                              defaults=defaults,
                                              usages=usages,
    ...
    def get_domain_project_quotas(self, context, project_id, quota_class=None,
                           defaults=True, usages=True, remains=False):
        """Retrieve the quotas for the given project.
        ...
        return self._driver_domain.get_project_quotas(context, self._resources,
                                              project_id,
                                              quota_class=quota_class,
                                              defaults=defaults,
                                              usages=usages,
    ...
    def get_settable_quotas_for_domain(self, context, domain_id, project_id=None, user_id=None):
        """
        Given a list of resources, retrieve the range of settable quotas for
        the given domain or project or user.
        ...
        return self._driver_domain.get_settable_quotas(context, self._resources,
                                                domain_id, project_id=project_id,
                                                user_id=user_id, usages=True, remains=True)
    ...
    def destroy_all_by_project_and_user_in_domain(self, context, project_id, user_id):
        """
        Destroy all quotas, usages, and reservations associated with a
        project and user.
        ...
        self._driver_domain.destroy_all_by_project_and_user(context,
                                                     project_id, user_id)
    ...
    def destroy_all_by_project_in_domain(self, context, project_id):
        """
        Destroy all quotas, usages, and reservations associated with a
        project.
        ...
        self._driver_domain.destroy_all_by_project(context, project_id)
    ...
    def destroy_all_by_domain(self, context, domain_id):
        ...
        self._driver_domain.destroy_all_by_domain(context, domain_id)
